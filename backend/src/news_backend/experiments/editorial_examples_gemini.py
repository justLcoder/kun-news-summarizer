...
from news_backend.experiments.editorial_examples import (
    INSTRUCTIONS, MAX_OUTPUT_TOKENS, build_prefix, load_references, select_targets,
)
...
def _report(summary, response):
    ...
    for label, attribute in (
        ...
        print(f'{label}: {value if type(value) is int else "unavailable"}', flush=True)


def run_experiment(factory, client, references, *, model: str, limit=5, article_ids=None):
    prefix = types.Content(role='user', parts=[types.Part.from_text(text=build_prefix(references))])
    ...
    for identifier, title, content in targets:
        ...
        try:
            response = client.models.generate_content(
                model=model,
                contents=[prefix, types.Content(role='user', parts=[types.Part.from_text(
                    text=f'TITLE:\n{title}\n\nARTICLE:\n{content}')])],
                config=config,
            )
...
def main():
    ...
    with make_client() as client:
        engine = make_engine()
        try:
            run_experiment(make_session_factory(engine), client, references,
                           model=args.model, limit=args.limit, article_ids=args.article_ids)
        finally:
            engine.dispose()
